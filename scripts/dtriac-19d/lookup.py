...
were annotated. The thrid file has all the wiki titles generated by the grunding
component.

TODO:
- experiment with what titles to use (those with 10 or more occurrences?)
...
def _lookup_technologies_in_tokens(lif, tokens, tex_view):
    ...
    for i in range(len(tokens)):
        ...
        for w, length in pairs:
            ...
            elif w in TECHNOLOGIES.wiki_titles:
                anno = _create_annotation(lif, tokens, w, i, length, "wiki_title")
                tex_view.annotations.append(anno)
    ...
    def __init__(self):
        ...
        for line in open(TECHNOLOGY_LIST):
            ...
            if not self.filter(term) and not term in self.stoplist:
                self.terms.add(term)
        for line in open(WIKI_TITLES):
            count, term = line.strip().split(' ', 1)
            count = int(count.strip())
            if (count >= MIN_WIKI_TITLE_COUNT
                and len(term.split()) <= LONGEST_TECHNOLOGY):
                self.wiki_titles.add(term)

    def __len__(self):
        return len(self.terms)

    def __str__(self):
        return "<TechnologyOntology terms=%d wiki_titles=%d>" \
            % (len(self.terms), len(self.wiki_titles))
